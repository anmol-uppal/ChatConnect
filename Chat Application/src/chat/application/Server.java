...
public class Server  implements ActionListener {
    ...
    JPanel a1; //declared globally since it has to be used in another function also
    //the below vertical fxn has been static bcz it is non static fxn and it cann0t be invoked inside the static(main) method
    static Box vertical = Box.createVerticalBox(); //messages would be shown in vertical manner
    static JFrame f=new JFrame(); //creating static object for the JFrame class bcz 'validate();' is non staic fxn and be invoked on JFrame as such
    static DataOutputStream dout; //for sending msgs
    ...
    Server() {
        ...
        ImageIcon i4 = new ImageIcon(ClassLoader.getSystemResource("icons/p1.png"));
        ...
        JLabel name = new JLabel("Amrita"); //JLabel used to display text on the panel
        ...
        f.setLocation(200, 50); //this will set new location for the frame which would be 200 from y-axis and 50 from x-axis
    ...
    //overriding the abstract method inside the ActionListener interface
    @Override
    public void actionPerformed(ActionEvent ae) {
        try{
            ...
            dout.writeUTF(out); //for sending the msgs to the client class
    ...
    public static void main(String[] args) {
        new Server(); //this is the anonymous object of class

        try {
            ServerSocket skt = new ServerSocket(6001);//port from where the opertaion begins
            Socket s = skt.accept();
            ...
            //for infintely getting and sending msgs
            //readUTF method is used (protocols being followed)
            while (true) {
